...
from linearity_analysis.px4_a1_family_readiness import run_px4_a1_family_readiness
...
def test_px4_a1_family_readiness_succeeds_on_synthetic_attitude_family(tmp_path: Path) -> None:
    ...
    summary = json.loads((study_dir / "summary" / "a1_family_readiness.json").read_text(encoding="utf-8"))

    assert summary["overall_decision"]["ready_for_reproduction_v1"] is True
    assert summary["overall_decision"]["recommended_path"] == "continue_px4_a1_attitude_reproduction"
    assert summary["selected_family"] == "attitude_roll_pitch_continuation"


def test_px4_a1_family_readiness_real_artifacts(tmp_path: Path) -> None:
    ...
    study_dir = run_px4_a1_family_readiness(output_root=tmp_path / "studies")
    summary = json.loads((study_dir / "summary" / "a1_family_readiness.json").read_text(encoding="utf-8"))

...
SKIP_CAPTURE=0
MATRIX_DIR=""
ANALYSIS_CONFIG="${ROOT_DIR}/configs/studies/ardupilot_generalization_diagnostic_matrix_analysis.yaml"
ABLATION_PLAN="${ROOT_DIR}/configs/ablations/ardupilot_generalization_diagnostic_matrix.yaml"
...
Run the ArduPilot generalization diagnostic matrix.
...
axis_paths = [
    root / "configs/studies/ardupilot_diagnostic_stabilize_axis_capture.yaml",
    root / "configs/studies/ardupilot_diagnostic_guided_nogps_axis_capture.yaml",
]
throttle_paths = [
    root / "configs/studies/ardupilot_diagnostic_stabilize_throttle_capture.yaml",
...
for base_index, base_path in enumerate(axis_paths):
    ...
    for axis_index, axis in enumerate(axes):
        for tier_index, (tier_name, factor) in enumerate(tiers):
            ...
            sweep_item["study_name"] = f"ardupilot_generalization_{mode_label}_nominal_sweep_{axis}_{tier_name}"
            sweep_item["scenario"] = "nominal"
            sweep_item["config_profile"] = f"ardupilot_generalization_{mode_label}_nominal_sweep_{axis}_{tier_name}"
            ...
            random_item["study_name"] = f"ardupilot_generalization_{mode_label}_dynamic_random_{axis}_{tier_name}"
            random_item["scenario"] = "dynamic"
            random_item["config_profile"] = f"ardupilot_generalization_{mode_label}_dynamic_random_{axis}_{tier_name}"
...
for base_index, base_path in enumerate(throttle_paths):
    ...
    for scenario_index, scenario_name in enumerate(("nominal", "throttle_biased")):
        for tier_index, (tier_name, factor) in enumerate(tiers):
            ...
            item["study_name"] = f"ardupilot_generalization_{mode_label}_{scenario_name}_alternating_throttle_{tier_name}"
            item["scenario"] = scenario_name
            item["config_profile"] = f"ardupilot_generalization_{mode_label}_{scenario_name}_alternating_throttle_{tier_name}"
            ...
            if input_type == "manual":
                item["extras"]["ardupilot_manual_throttle_bias"] = 0.68 if scenario_name == "throttle_biased" else 0.65

...
SKIP_CAPTURE=0
MATRIX_DIR=""
ANALYSIS_CONFIG="${ROOT_DIR}/configs/studies/ardupilot_real_stabilize_partial_baseline_analysis.yaml"
ABLATION_PLAN="${ROOT_DIR}/configs/ablations/ardupilot_stabilize_partial_baseline.yaml"

usage() {
  cat <<'EOF'
Usage: run_ardupilot_stabilize_partial_baseline.sh [--vehicle ArduCopter] [--frame quad] [--repeat 1]
                                                   [--accepted-target 5] [--max-attempts-per-config N]
...
run_stage_check partial-baseline \

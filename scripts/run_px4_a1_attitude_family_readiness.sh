...
usage() {
  cat <<'EOF'
Usage: run_px4_a1_attitude_family_readiness.sh [--baseline-study DIR] [--diagnostic-study DIR] [--output-root DIR]

Run the PX4 A1 attitude-family readiness analysis on canonical or user-provided study artifacts.
...
ARGS=(--family attitude_roll_pitch_continuation)
...
ANALYSIS_OUTPUT="$(run_py 'from linearity_analysis.px4_a1_family_readiness import main; main()' "${ARGS[@]}")"
printf '%s\n' "${ANALYSIS_OUTPUT}"

...
usage() {
  cat <<'EOF'
Usage: run_ardupilot_a2_stabilize_boundary_readiness.sh [--vehicle ArduCopter] [--frame quad]
                                                        [--skip-sitl] [--skip-capture --matrix-dir DIR]
                                                        [--output-root DIR]
                                                        [--scenarios nominal,throttle_biased]

Run the narrow ArduPilot A2 STABILIZE boundary-readiness study.
- fixed scope: STABILIZE only, throttle only, pulse_train only
...
specs = [
    ("nominal", "micro", 0.01, 0.0, 0.55, 0.05, 5, 0.35, 0.95),
    ("nominal", "probe", 0.02, 0.0, 0.55, 0.05, 5, 0.35, 0.95),
    ("nominal", "confirm", 0.04, 0.0, 0.55, 0.05, 5, 0.35, 0.95),
    ("throttle_biased", "micro", 0.01, 0.04, 0.60, 0.05, 5, 0.35, 0.95),
    ("throttle_biased", "probe", 0.02, 0.04, 0.60, 0.05, 5, 0.35, 0.95),
    ("throttle_biased", "confirm", 0.04, 0.04, 0.60, 0.05, 5, 0.35, 0.95),
]

for index, (scenario, tier, amplitude, bias, manual_bias, manual_scale, pulse_count, pulse_width_s, pulse_gap_s) in enumerate(specs):
    if scenario_filter and scenario not in scenario_filter:
        continue
    item = copy.deepcopy(payload)
    item["study_name"] = f"ardupilot_a2_stabilize_boundary_readiness_{scenario}_{tier}"
    item["scenario"] = scenario
    item["config_profile"] = f"ardupilot_a2_stabilize_boundary_readiness_{scenario}_{tier}"
    item["seed"] = base_seed + index
    item["axis"] = "throttle"
    item["profile_type"] = "pulse_train"
    item["amplitude"] = amplitude
    item["bias"] = bias
    item["perturbation_strategy"] = "a2_boundary_readiness_v4"
    ...
    item["extras"]["readiness_protocol_version"] = "v4"
